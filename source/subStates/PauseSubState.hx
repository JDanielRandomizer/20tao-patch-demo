...
class PauseSubState extends MusicBeatSubState
{
	var optionShit:Array<String> = [
		...
		"restart song",
		// "botplay",
		// "options", // tive que remover devido a modcharts bugarem com isso ativo, desculpa
		"exit to menu",
	];
	...
	public function new()
	{
		...
		var textArray:Array<String> = [
			FreeplayState.songSelectedDisplay,
			PlayState.songDiff,
	...
	function closePause()
	{
		...
		if(SaveData.data.get('Countdown on Unpause'))
			...
			if(PlayState.cameraZoomTween != null)
				PlayState.cameraZoomTween.active = false;
	...
	override function update(elapsed:Float)
	{
		...
		if(PlayState.SONG.song == 'papai-calabreso' && FlxG.fullscreen)
			...
			if(Controls.justPressed(ACCEPT))
			{
				switch(optionShit[curSelected])
				{
					...
					case "options":
						...
						pauseSong.pause();
						this.openSubState(new OptionsSubState(true));
						PlayState.desgraca();
