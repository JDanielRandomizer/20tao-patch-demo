...
{	
	...
	public function new(fadeOut:Bool = true, transition:String = "20tao")
	{
		super();
		...
		switch(transition) {
			...
			case '20tao':
				sprBlack = new FlxSprite(0, !fadeOut ? -720 : 0, Paths.image('cedula'));
				sprBlack.scrollFactor.set();
				add(sprBlack);

				FlxTween.tween(sprBlack, {y: !fadeOut ? 0 : 720}, !fadeOut?0.4:0.8, { onComplete: function(twn:FlxTween) { endTransition(); }, ease: FlxEase.expoInOut});
		}
	}
